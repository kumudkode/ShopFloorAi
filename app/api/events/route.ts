  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: timestamp, worker_id, workstation_id, event_type",
        },
        { status: 400 },
      );
    }
    ...
    if (body.event_type === "product_count") {
      if (typeof body.count !== "number" || body.count < 0) {
        return NextResponse.json(
          {
            success: false,
            error: "product_count events require a non-negative count value",
          },
          { status: 400 },
        );
      }
    ...
    if (!getWorkstationById(body.workstation_id)) {
      return NextResponse.json(
        {
          success: false,
          error: `Workstation ${body.workstation_id} not found`,
        },
        { status: 404 },
      );
    }
