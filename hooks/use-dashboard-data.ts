...
import type { DashboardData, WorkerMetrics, WorkstationMetrics, FactoryMetrics } from "@/frontend/types"
...
export function useWorkers() {
  ...
  const workers: Worker[] | undefined = data?.workers.map((w: WorkerMetrics) => ({
    id: w.worker_id,
    name: w.name,
...
export function useFactoryKPIs() {
  ...
  const kpis = {
    ...
    idleHours: Math.round((factory.total_workers * 8 * 60 - factory.total_productive_time_minutes) / 60),
